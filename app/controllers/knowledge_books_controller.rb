...
class KnowledgeBooksController < ApplicationController
  ...
  before_action :set_knowledge_book, only: %i[show edit update destroy]
  ...
  def knowledge_book_params
    params.require(:knowledge_book).permit(:code, :title, :content,
                                           knowledge_chapters_attributes: %i[id title content _destroy])
  end
end
