class LanguageDatatable < AjaxDatatablesRails::Base
  ...
  def data
    ...
    ops = ''
      ...
      ops = ops + " " + link_to('<i class="fas fa-pencil-alt"></i>'.html_safe, edit_language_path(record), remote: true) 
      end
  
      if options[:ca].can? :destroy, record then       
        ops = ops + " " + link_to('<i class="fa fa-times-circle"> </i>'.html_safe, language_path(record), method: :delete, remote: true, data: { confirm: 'Are you sure you want to delete this language?' }) 
    end  
