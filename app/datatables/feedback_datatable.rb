class FeedbackDatatable < AjaxDatatablesRails::Base
  ...
  def data
    ...
    ops = ''
    if options[:ca].can? :show, record then 
      ...
      if options[:ca].can? :destroy, record then       
        ops = ops + " " + link_to('<i class="fa fa-times-circle"> </i>'.html_safe, feedback_path(record), method: :delete, remote: true, data: { confirm: 'Are you sure you want to delete this feedback?' }) 
    end  
