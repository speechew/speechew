  ...
  def data
    records.map do |record|
      ...
      if options[:ca].can? :destroy, record then       
        ops = ops + " " + link_to('<i class="fa fa-times-circle"> </i>'.html_safe, room_path(record), method: :delete, remote: true, data: { confirm: 'Are you sure you want to delete this room?' }) 
	  end  
