  ...
  def data
    records.map do |record|
      ...
      if options[:ca].can? :update, record
        ops = ops + ' ' + link_to('<i class="fas fa-pencil-alt"> </i>'.html_safe, edit_country_path(record),
                                  remote: true)
      end

      if options[:ca].can? :destroy, record
        ops = ops + ' ' + link_to('<i class="fa fa-times-circle"> </i>'.html_safe, country_path(record),
                                  method: :delete, remote: true, data: { confirm: 'Are you sure you want to delete this country?' })
      end
